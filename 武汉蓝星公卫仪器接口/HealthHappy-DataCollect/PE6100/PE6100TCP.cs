...
namespace PE6100
{
    ...
    /// 通讯：TCP/IP协议，单向
    /// </summary>
    public class PE6100TCP : IDataCollect
    {
            ...
            if (!string.IsNullOrEmpty(DataRaw) && DataRaw.IndexOf(char28) >= 0)
            {
                ...
                while (!string.IsNullOrEmpty(DataRaw) && DataRaw.IndexOf('\r') >= 0)
                {
                    ...
                    if (!string.IsNullOrEmpty(data))
                    {
                        ...
                        else if (line[0].IndexOf("OBX") >= 0 && line.Length > 10)//确保数据完整性
                        {
                            if (!string.IsNullOrEmpty(sampleno))
                            {
                                ...
                                instr_codes = line[3].Split('^');
                                if (instr_codes.Length == 3)
                                {
                                    SQLHelp.UpdateItem(sampleno, AppConfig.Instance.GroupId, instr_codes[1], line[5], result_mark, line[6], ref_low, ref_high, line[7]);
                                    BaseDeal.DataLog(DateTime.Now.ToString() + "  " + "样本号:" + sampleno + "，项目代号:" + AppConfig.Instance.GroupId + "，项目仪器编码:" + instr_codes[1] + " " + "，项目结果:" + line[5] + "\r\n");
                                }
                            }
                        }
                    }
